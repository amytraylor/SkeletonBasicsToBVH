...
// This structure implies the basic skeleton data each frame like a queue(however, we will not use all abilities of queue, but some basic ones with some differences).
// The firstPoint don't contain any skeleton data. It's just made for pointing to the skeleton data in first frame.
struct b_Skeleton {
	NUI_SKELETON_FRAME skeletonFrameData;
	b_Skeleton *nextFrame;
};
...
struct skeletonJoint {
	std::string bvhJointName;
	int kinectJointName;
	skeletonJoint *firstChild, *nextSibling;
};
...
HRESULT CSkeletonBasics::CreateFirstConnected()
{
    ...
    if (NULL != m_pNuiSensor)
    {
        ...
        if (SUCCEEDED(hr))
        {	
			// Create the firstPoint
			firstPoint = (struct b_Skeleton*) malloc(sizeof(struct b_Skeleton));
...
void CSkeletonBasics::ProcessSkeleton()
{
	...
	latestFrameSkeleton = latestFrameSkeleton->nextFrame;
...
std::string CSkeletonBasics::getJointStringForBVH(NUI_SKELETON_DATA firstSkeletonData,int jointNo, std::string indent, int jointParrentNo, const std::vector<std::list<int>> & jointList) {
	...
	if (jointList[jointNo].empty()) {
		...
		if (indent.empty()) {
			stringResult += ("ROOT " + toBVHJointName[jointNo] + '\n');
		}
		else {
			stringResult += (indent + "JOINT " + toBVHJointName[jointNo] + '\n');
		}
		stringResult += (indent + '{' + '\n');
		stringResult += (nextIndent + "OFFSET " + s_jointX + ' ' + s_jointY + ' ' + s_jointZ + '\n');
		if (indent.empty()) {
			stringResult += (nextIndent + "CHANNELS 6 Xposition Yposition Zposition Zrotation Xrotation Yrotation\n");
		}
		else {
...
void CSkeletonBasics::PrintSkeletonAsBvhFile()
{
	...
	int n, m;
	std::string bVHName;
	std::list<int> _temp_list;
	int _temp_Joint;
	customHierachy >> n;
	for (int i = 0; i < n; i++) {
		customHierachy >> bVHName;
		toBVHJointName[i] = bVHName;
		customHierachy >> m;
		_temp_list.clear();
		for (int j = 0; j < m; j++) {
